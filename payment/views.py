        ...
        if coupon.can_use():
            ...
            for item in cart:
                ...
                if product.available <= 0:
                    return Response(data={'message': 'One of your products is sold out'}, status=409)
...
def redeem_coupon(request):
    ...
    if request.method == methods.post:
        if coupon.can_use():
            coupon.used()
            return Response({'discount': coupon.discount, 'message': "discount successfully used"}, safe=False)
